...
var reg = /(^[0-9]\d{0,2}$)/;     
   if(reg.test(v.seatnumber.value) === false)  
   {  
       alert("实验室座位数请输入三位纯数字！");
        v.seatnumber.focus();  
       return  false;  
   } 
   ...
   var regs = /(^[0-9]\d{0,2}$)/;     
   if(v.expense.value!=""&&regs.test(v.expense.value) === false)  
   {  
       alert("实验室费用请输入纯数字！");
        v.expense.focus();  
       return  false;  
      ...
      var regss = /(^\d{2}-(\d{3})$)/;     
   if(regss.test(v.number.value) === false)  
   {  
       alert("实验室号请按照01-101格式填写！");
        v.number.focus();  
       return  false;  
   } 
    ...
    function chg(obj)
    {
        if(obj.options[obj.selectedIndex].value =="科研研究")
...
.table-a table td{border-left:0;border-top:1px solid skyblue;height:40px}
 ...
 <a href="index.jsp" style="text-decoration:none;color:white;"><img src="sysimages/relogin.png" style="width:20px;height:20px">退出登录</a>
  ...
  <div id="top">
  当前位置：<a href="ShowLabServlet?school=<%=session.getAttribute("school")%>" style="text-decoration:none;color:white;">实验室信息管理</a> > 添加实验室信息
   ...
   <tr><td colspan="5"id="myfont1" style="background:url(images/a.jpg);height:30;">添加实验室信息</td></tr>
   </table>
   </form>
   
  <form action="LabServlet?school=<%=session.getAttribute("school")%>" method="post" onSubmit="return validate(this)" enctype="multipart/form-data">
  <table border="0">
  
                <tr>
				<td align="center">实验室号:</td>
				<td><input  type="text" name="number" id="myinput"></td>
				<td align="center">实验室管理员:</td>
				<td><input type="text" name="lableader" id="myinput" value="<%=request.getParameter("name")%>" readonly="true" style="color:gray"></td>
				</tr>			

                <tr><td align="center">管理员手机:</td>
                <td><input type="text" name="lableaderphone" id="myinput" value="<%=request.getParameter("phone")%>" readonly="true" style="color:gray"></td></td>
                </tr>
			
			    <tr>			
				<td  align="center">开放时间:</td>
				<td colspan="3">			
				   <input type="checkbox" name="opentime1" value="8:00-10:00" checked> 8:00-10:00  
				   ...
				   <input type="checkbox" name="opentime4" value="16:10-17:30"> 16:10-17:30 
				</td>				
			    </tr>	
				...
				<td><input type="text" name="seatnumber" id="myinput"></td>
				<td  align="center">设备及数量:</td>
			    <td><input type="text" name="equipment" id="myinput"></td>
			   </tr>
			   
			     <tr>
				<td align="center">实验室类型:</td>
				 <td>
				 <select  onchange="chg(this)" name="type" id="myinput">
				 <option value="教学需要" selected>教学需要</option>
				 <option value="科研研究">科研研究</option>
				 </select>
                 </td>
	             <td align="center">实验室学校:</td>  
	             <td><input  type="text" name="labschool" id="myinput"  value="<%=session.getAttribute("school")%>"readonly="true" style="color:gray"></td>     
	             </tr>
	            
	             <tr id="select_else" style="display:none">
				 <td  align="center">实验室费用:</td>
			     <td><input type="text" name="expense" id="myinput"></td>
			     <td colspan="2" style="color:red">*当实验室为科研研究类型时才收取实验室费用</td>
			    </tr>
			    
			    <tr>
				<td align="center">上传图片1:</td>
				<td><input type="file" name="picture1" id="doc" onchange="javascript:setImagePreview();" style="border: skyblue 1px solid; height: 35px;"></td>
				 <td align="center">上传图片2:</td>
				<td><input type="file" name="picture" id="doc1" onchange="javascript:setImagePreview1();" style="border: skyblue 1px solid; height: 35px;"></td>
			...
			<tr>
				<td colspan="4">
				<center><input id="mybutton" type="submit" value="添加" name="btnAdd"/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
