...
.table-a table td{border-left:0;border-top:1px solid skyblue;height:40px}
 ...
 <a href="index.jsp" style="text-decoration:none;color:white;"><img src="sysimages/relogin.png" style="width:20px;height:20px">退出登录</a>
  ...
  当前位置：我已审核管理  
   ...
   <center>
   <form class="table-c" action="SelectLeaVerifyResultServlet?school=<%=session.getAttribute("school")%>" method="post">
   ...
   <tr style="font-weight: bold;">
       ...
       <td align="center">审核结果</td>
        ...
        <% ArrayList<Application> applications=(ArrayList<Application>)application.getAttribute("applications");	
			if(applications!=null){
	          	for(int i=0;i<applications.size();i++)
	         	 {
	            	 ...
	            	 %>
	            	 <tr>
	            	 <td><center><%=appl.getNumber()%></center></td>
	            	 <td><center><%=appl.getOpentime1()%> <%=appl.getOpentime2()%> <%=appl.getOpentime3()%> <%=appl.getOpentime4()%></center></td>
	            	 <td><center><%=appl.getSeatnumber()%></center></td>
	            	 <td><center><%=appl.getEquipment()%></center></td>
	            	 <td><center><%=appl.getName()%></center></td>
	            	 <td><center><%=appl.getPhone()%></center></td>
	            	 <td><center><%=appl.getType()%></center></td>
	            	 <td><center><%=appl.getTeaschool()%></center></td>
	            	 <td><center><%=appl.getDate()%> <%=appl.getExperimenttime()%></center></td>
	            	 <td><center><%=appl.getApplytime()%></center></td>
	            	 <td><center><%=appl.getResult()%></center></td>
	            	 <td><center><%=appl.getReason()%></center></td>
	            	 </tr>
	            	 <%
	             }
	         } %>
		...
		<%-- 	[<button style="border:0;background-color: ;"  onclick="javascript:to_page(${Page.currpageno-1})">前一页</button>] --%>
			[<a href="ShowLeaVerifyResultServlet?page=${Page.currpageno-1}&&school=${sessionScope.school}" style="text-decoration:none">前一页</a>]
			
		<%-- 	[<button style="border:0;background-color: ;" onclick="javascript:to_page(${Page.currpageno+1})">后一页</button>] --%>
 [<a href="ShowLeaVerifyResultServlet?page=${Page.currpageno+1}&&school=${sessionScope.school}" style="text-decoration:none">后一页</a>]
