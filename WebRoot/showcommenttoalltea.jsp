...
.table-a table td{border-left:0;border-top:1px solid skyblue;height:40px}
 ...
 <a href="index.jsp" style="text-decoration:none;color:white;"><img src="sysimages/relogin.png" style="width:20px;height:20px">退出登录</a>
  ...
  当前位置：<a href="ShowTeaLabServlet" style="text-decoration:none;color:white;">申请实验室</a> > 实验室评论 
...
<img src="sysimages/user.png" style="width:30px;height:30px"> <%=session.getAttribute("username")%></td></tr>
   ...
   <tr><td colspan="4" id="myfont1" style="background:url(images/a.jpg);height:30;">评论信息</td></tr>

       
       <tr style="font-weight: bold;">
       <td align="center">评论内容</td>
       <td align="center">评论账户</td>
       <td align="center">评论时间</td>
       <td align="center">评论图片</td>
       </tr>
			...
			if(comments!=null){
	          	for(int i=0;i<comments.size();i++)
	         	 {
	            	 ...
	            	 %>
	            	 <tr>	      
	            	 <td><center><%=comment.getContent()%><input type="hidden" value="<%=comment.getId()%>"></center></td>
	            	 <td><center><%=comment.getUsername()%></center></td>
	            	 <td><center><%=comment.getCommenttime()%></center></td>
	            	 <td align="center"><a href="ShowLabCommentPictureToOtherTeacherServlet?id=<%=comment.getId()%>" style="text-decoration:none;color:black;"><img src="sysimages/photos.png" style="width:20px;height:20px"></a></td>		            		         
	            	 </tr>
	            	 <%
	             }
	         } %>
...
[<a href="ShowCommentToAllTeaServlet?page=${Page.currpageno-1}&&number=${sessionScope.number}&&labschool=${sessionScope.labschool}" style="text-decoration:none">前一页</a>]
			
		<%-- 	[<button style="border:0;background-color: ;" onclick="javascript:to_page(${Page.currpageno+1})">后一页</button>] --%>
 [<a href="ShowCommentToAllTeaServlet?page=${Page.currpageno+1}&&number=${sessionScope.number}&&labschool=${sessionScope.labschool}" style="text-decoration:none">后一页</a>]

</td></tr>
	         
	         
	         <tr><td colspan="7" align="center"><input type="button" value="返回" id="mybutton" onclick="location.href='ShowTeaLabServlet'"></td></tr>
   </table>
   </form>
   </center>
   </div>
