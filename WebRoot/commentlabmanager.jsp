...
.table-a table td{border-left:0;border-top:1px solid skyblue;height:40px}
  ...
  当前位置：我的评论管理
   ...
   <center>
   <form class="table-c" action="SelectTeaCommentServlet?username=<%=session.getAttribute("username")%>" method="post">
   ...
   <tr><td colspan="7" id="myfont1" style="background:url(images/a.jpg);height:30;">评论信息</td></tr>
   <tr style="font-weight: bold;">
       ...
       <td align="center">操作</td>
			 ...
			 <% ArrayList<Comment> comments=(ArrayList<Comment>)application.getAttribute("comments");	
			if(comments!=null){
	          	for(int i=0;i<comments.size();i++)
	         	 {
	            	 ...
	            	 %>
	            	 <tr>
	            	 <td><center><%=comment.getNumber()%><input type="hidden"name="id" value="<%=comment.getId()%>"></center></td>
	            	 <td><center><%=comment.getLabschool()%></center></td>
	            	 <td><center><%=comment.getExperimentdate()%> <%=comment.getExperimenttime()%></center></td>
	            	 <td><center><%=comment.getContent()%></center></td>	            	 
	            	 <td><center><%=comment.getCommenttime()%></center></td>	
	            	 <td><center><a href="ShowTeaCommentPictureServlet?id=<%=comment.getId()%>" style="text-decoration:none;color:black;"><img src="sysimages/photos.png" style="width:20px;height:20px"></a></center></td>            		         
	            	 <td align="center">
	            	 <a href="DeleteCommentServlet?id=<%=comment.getId()%>&&username=<%=session.getAttribute("username")%>"style="text-decoration:none;color:black;" onclick="return confirm('您确定要删除此条信息吗？');"><img src="sysimages/delete.png" style="width:20px;height:20px"></a>
	            	 </td>
	            	 </tr>
	            	 <%
	             }
...
总共[<B>${Page.totalcount}</B>]记录   共[<B>${Page.totalPagecount}</B>]页
		...
		<%-- 	[<button style="border:0;background-color: ;"  onclick="javascript:to_page(${Page.currpageno-1})">前一页</button>] --%>
			[<a href="ShowTeaCommentServlet?page=${Page.currpageno-1}&&username=${sessionScope.username}" style="text-decoration:none">前一页</a>]
			
		<%-- 	[<button style="border:0;background-color: ;" onclick="javascript:to_page(${Page.currpageno+1})">后一页</button>] --%>
 [<a href="ShowTeaCommentServlet?page=${Page.currpageno+1}&&username=${sessionScope.username}" style="text-decoration:none">后一页</a>]
