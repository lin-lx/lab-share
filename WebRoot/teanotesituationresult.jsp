...
.table-a table td{border-left:0;border-top:1px solid skyblue;height:40px}
 ...
 当前位置：我的反馈信息
   ...
   <center>
   <form class="table-c" action="SelectTeaNoteResultServlet?username=<%=session.getAttribute("username")%>" method="post">
   ...
   <tr><td colspan="10"id="myfont1" style="background:url(images/a.jpg);height:30;">检查信息</td></tr>
         ...
         <% ArrayList<Note> notes=(ArrayList<Note>)application.getAttribute("notes");	
			if(notes!=null){
	          	for(int i=0;i<notes.size();i++)
	         	 {
        	         ...
        	         <td><center><%=note.getLabschool()%></center></td>
        	         <td><center><%=note.getEnvironment()%></center></td>
        	         <td><center><%=note.getDamage()%></center></td>
        	         <td><center><%=note.getDamagedescribe()%></center></td>
	            	 <td><center><%=note.getNotename()%></center></td>
	            	 <td><center><%=note.getNotetime()%></center></td>
	            	 <td><center><a href="ShowTeaNoteResultPictureServlet?number=<%=note.getNumber()%>&&labschool=<%=note.getLabschool()%>&&experimenttime=<%=note.getExperimenttime()%>&&experimentdate=<%=note.getExperimentdate()%> " style="text-decoration:none;color:black;"><img src="sysimages/photos.png" style="width:20px;height:20px"></a></center></td>
	            	 </tr>
	            	 <%
	             }
	         } %>
...
总共[<B>${Page.totalcount}</B>]记录   共[<B>${Page.totalPagecount}</B>]页
		...
		<%-- 	[<button style="border:0;background-color: ;"  onclick="javascript:to_page(${Page.currpageno-1})">前一页</button>] --%>
			[<a href="TeaNoteSituationServlet?page=${Page.currpageno-1}&&username=${sessionScope.username}" style="text-decoration:none">前一页</a>]
			
		<%-- 	[<button style="border:0;background-color: ;" onclick="javascript:to_page(${Page.currpageno+1})">后一页</button>] --%>
 [<a href="TeaNoteSituationServlet?page=${Page.currpageno+1}&&username=${sessionScope.username}" style="text-decoration:none">后一页</a>]
