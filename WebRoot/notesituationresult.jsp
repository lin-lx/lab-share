...
<a href="manfirst.jsp" style="text-decoration:none;color:white;"><img src="sysimages/first.png" style="width:20px;height:20px">首页</a>
  ...
  当前位置：发送赔偿消息
   ...
   <center>
   <form class="table-c" action="SelectManNoteResultServlet?school=<%=session.getAttribute("school")%>" method="post">
   ...
   <tr style="font-weight: bold;">
       ...
       <td align="center">操作</td>
         ...
         <% ArrayList<Note> notes=(ArrayList<Note>)application.getAttribute("notes");	
			if(notes!=null){
	          	for(int i=0;i<notes.size();i++)
	         	 {
	            	 ...
	            	 <td align="center">
	            	 <a href="ToSendCompensationServlet?number=<%=note.getNumber()%>&&experimenttime=<%=note.getExperimenttime()%>&&experimentdate=<%=note.getExperimentdate()%>&&labschool=<%=note.getLabschool()%>&&username=<%=session.getAttribute("username") %>" style="text-decoration:none;color:black;"><img src="sysimages/comment.png" style="width:20px;height:20px">发送赔偿</a></td>
	            	 </tr>
	            	 <%
	             }
	         } %>
		...
		<%-- 	[<button style="border:0;background-color: ;"  onclick="javascript:to_page(${Page.currpageno-1})">前一页</button>] --%>
			[<a href="ShowManToSendCompensationServlet?page=${Page.currpageno-1}&&school=${sessionScope.school}" style="text-decoration:none">前一页</a>]
			
		<%-- 	[<button style="border:0;background-color: ;" onclick="javascript:to_page(${Page.currpageno+1})">后一页</button>] --%>
 [<a href="ShowManToSendCompensationServlet?page=${Page.currpageno+1}&&school=${sessionScope.school}" style="text-decoration:none">后一页</a>]
