...
.table-a table td{border-left:0;border-top:1px solid skyblue;height:40px}
 ...
 当前位置：教师信誉评分管理
   ...
   <center>
   <form class="table-c" action="SelectManFinishCreditServlet?school=<%=session.getAttribute("school")%>" method="post">
   ...
   <tr style="font-weight: bold;">
       ...
       <td align="center">设备评分</td>
       <td align="center">卫生评分</td>
        <td align="center">扣分理由</td>
       <td align="center">记录时间</td> 
       <td align="center">图片</td>
       </tr>
         <% ArrayList<Credit> credits=(ArrayList<Credit>)application.getAttribute("credits");	
			if(credits!=null){
	          	for(int i=0;i<credits.size();i++)
	         	 {
	            	 ...
	            	 %>
	            	 <tr>
	            	 <td><center><%=credit.getNumber()%></center></td>
        	         <td><center><%=credit.getName()%></center></td>
        	         <td><center><%=credit.getExperimentdate()%> <%=credit.getExperimenttime()%></center></td>
        	         <td><center><%=credit.getTeaschool()%></center></td>
        	         <td><center><%=credit.getDamage()%></center></td>
        	         <td><center><%=credit.getEnvironment()%></center></td>
        	         <td><center><%=credit.getReason()%></center></td>
        	         <td><center><%=credit.getNotetime()%></center></td>
	            	 <td><center><a href="ShowManTeaCreditPictureServlet?number=<%=credit.getNumber()%>&&school=<%=session.getAttribute("school")%>&&experimenttime=<%=credit.getExperimenttime()%>&&experimentdate=<%=credit.getExperimentdate()%>"style="text-decoration:none;color:black;"><img src="sysimages/photos.png" style="width:20px;height:20px"></a></center></td>
		...
		<%-- 	[<button style="border:0;background-color: ;"  onclick="javascript:to_page(${Page.currpageno-1})">前一页</button>] --%>
			[<a href="ShowManTeaCreditServlet?page=${Page.currpageno-1}&&school=${sessionScope.school}" style="text-decoration:none">前一页</a>]
			
		<%-- 	[<button style="border:0;background-color: ;" onclick="javascript:to_page(${Page.currpageno+1})">后一页</button>] --%>
 [<a href="ShowManTeaCreditServlet?page=${Page.currpageno+1}&&school=${sessionScope.school}" style="text-decoration:none">后一页</a>]
