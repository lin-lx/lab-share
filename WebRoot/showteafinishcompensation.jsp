...
.table-a table td{border-left:0;border-top:1px solid skyblue;height:40px}
 ...
 当前位置：我的赔偿记录
   ...
   <tr><td id="myfont1"><img src="sysimages/user.png" style="width:28px;height:30px"> <%=session.getAttribute("username")%></td></tr>
    <tr><td id="myfont1">>>欢迎进入本系统！</td></tr>
   <tr><td id="myfont1"><img src="sysimages/date.png" style="width:28px;height:30px"> <%
        Date date = new Date();
   ...
   <center>
   <form class="table-c" action="SelectTeaFinishCompensationServlet?username=<%=session.getAttribute("username")%>" method="post">
   ...
   <tr><td colspan="11" id="myfont1" style="background:url(images/a.jpg);height:30;">赔偿信息</td></tr>
			...
			if(compensations!=null){
	          	for(int i=0;i<compensations.size();i++)
	         	 {
        	         ...
        	         <td><center><%=compensation.getMoney()%></center></td>
        	          <td><center><%=compensation.getPayment()%></center></td>
...
总共[<B>${Page.totalcount}</B>]记录   共[<B>${Page.totalPagecount}</B>]页
		...
		<%-- 	[<button style="border:0;background-color: ;"  onclick="javascript:to_page(${Page.currpageno-1})">前一页</button>] --%>
			[<a href="ShowTeaFinishCompensationServlet?page=${Page.currpageno-1}&&username=${sessionScope.username}" style="text-decoration:none">前一页</a>]
			
		<%-- 	[<button style="border:0;background-color: ;" onclick="javascript:to_page(${Page.currpageno+1})">后一页</button>] --%>
 [<a href="ShowTeaFinishCompensationServlet?page=${Page.currpageno+1}&&username=${sessionScope.username}" style="text-decoration:none">后一页</a>]
