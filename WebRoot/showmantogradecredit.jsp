...
<img src="sysimages/user.png" style="width:30px;height:30px"> <%=session.getAttribute("username")%></td></tr>
   ...
   <center>
   <form class="table-c" action="SelectManToGradeCreditServlet?school=<%=session.getAttribute("school")%>" method="post">
   ...
   <tr><td colspan="5" id="myfont1" style="background:url(images/a.jpg);height:30;">实验室信息</td></tr>
         ...
         <% ArrayList<Note> notes=(ArrayList<Note>)application.getAttribute("notes");	
			if(notes!=null){
	          	for(int i=0;i<notes.size();i++)
	         	 {
	            	 ...
	            	 <td align="center">
	            	 <a href="ManToGradeCreditServlet?number=<%=note.getNumber()%>&&experimenttime=<%=note.getExperimenttime()%>&&school=<%=session.getAttribute("school")%>&&experimentdate=<%=note.getExperimentdate()%>&&labschool=<%=note.getLabschool()%>&&username=<%=session.getAttribute("username") %>" style="text-decoration:none;color:black;"><img src="sysimages/note.png" style="width:20px;height:20px">评分</a></td>
	            	 </tr>
	            	 <%
	             }
	         } %>
...
总共[<B>${Page.totalcount}</B>]记录   共[<B>${Page.totalPagecount}</B>]页
		...
		<%-- 	[<button style="border:0;background-color: ;"  onclick="javascript:to_page(${Page.currpageno-1})">前一页</button>] --%>
			[<a href="ShowManToGradeCreditServlet?page=${Page.currpageno-1}&&school=${sessionScope.school}" style="text-decoration:none">前一页</a>]
			
		<%-- 	[<button style="border:0;background-color: ;" onclick="javascript:to_page(${Page.currpageno+1})">后一页</button>] --%>
 [<a href="ShowManToGradeCreditServlet?page=${Page.currpageno+1}&&school=${sessionScope.school}" style="text-decoration:none">后一页</a>]
