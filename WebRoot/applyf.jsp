...
.table-a table td{border-left:0;border-top:1px solid skyblue;height:40px}
 ...
 <a href="index.jsp" style="text-decoration:none;color:white;"><img src="sysimages/relogin.png" style="width:20px;height:20px">退出登录</a>
  ...
  当前位置：我的申请管理
   ...
   <center>
   <form class="table-c" action="SelectTeaApplyfServlet?username=<%=session.getAttribute("username")%>" method="post">
   ...
   状态：<select id="myinput" name="result">
                  <option value=""></option>
       ...
       <td align="center" colspan="2">操作</td>
			...
			if(applications!=null){
	          	for(int i=0;i<applications.size();i++)
	         	 {
	            	 ...
	            	 <td><center><%=apply.getLableader()%></center></td>
	            	 <td><center><%=apply.getLableaderphone()%></center></td>
	            	 <td><center><%=apply.getResult()%></center></td>
                     <td><center><%=apply.getVerifyname()%></center></td>	            	 
	            	  <td align="center"><a name="btnAlt" href="TeacherCancelApplyServlet?number=<%=apply.getNumber()%>&&experimenttime=<%=apply.getExperimenttime()%>&&labschool=<%=apply.getLabschool()%>&&username=<%=session.getAttribute("username")%>&&date=<%=apply.getDate()%>&&id=<%=apply.getId()%>" style="text-decoration:none;color:black;"  onclick="return confirm('您确定要取消申请吗？');" >
	            	 	<img src="sysimages/cancel.png" style="width:15px;height:15px">取消申请</a>
	            	 </td>
	            	 <td align="center"><a name="btnAlt" href="TeaSendRemindVerifyServlet?number=<%=apply.getNumber()%>&&experimenttime=<%=apply.getExperimenttime()%>&&labschool=<%=apply.getLabschool()%>&&username=<%=session.getAttribute("username")%>&&date=<%=apply.getDate()%>" style="text-decoration:none;color:black;"  >
	            	 	<img src="sysimages/verifyremind.png" style="width:15px;height:15px">提醒审核</a>
	            	 </td>
	            	 </tr>
	            	 <%
	             }
   ...
   总共[<B>${Page.totalcount}</B>]记录   共[<B>${Page.totalPagecount}</B>]页
			...
			<%-- [<button style="border:0;background-color: ;"  onclick="javascript:to_page(${Page.currpageno-1})">前一页</button>] --%>
			 [<a href="ApplyLabServletTwo?username=${sessionScope.username}&&page=${Page.currpageno-1}" style="text-decoration:none">前一页</a>]

    		
			<%-- [<button style="border:0;background-color: ;" onclick="username=${sessionScope.username}&javascript:to_page(${Page.currpageno+1})">后一页</button>] --%>
[<a href="ApplyLabServletTwo?username=${sessionScope.username}&&page=${Page.currpageno+1}" style="text-decoration:none">后一页</a>]
