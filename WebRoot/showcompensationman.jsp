...
<img src="sysimages/date.png" style="width:30px;height:30px"> <%
   ...
   <center>
   <form class="table-c" action="SelectManCompensationServlet?school=<%=session.getAttribute("school")%>&&username=<%=session.getAttribute("username")%>" method="post">
   ...
   是否已付：<select id="myinput" name="pay">
 				<option value=""></option>
   ...
   <tr><td colspan="8" id="myfont1" style="background:url(images/a.jpg);height:30;">赔偿信息</td></tr>
   <tr style="font-weight: bold;">
       ...
       <td align="center">是否已付</td>
         ...
         <% ArrayList<Compensation> compensations=(ArrayList<Compensation>)application.getAttribute("compensations");	
			if(compensations!=null){
	          	for(int i=0;i<compensations.size();i++)
	         	 {
        	         ...
        	         <td><center><%=compensation.getName()%></center></td>
        	         <td><center><%=compensation.getExperimentdate()%> <%=compensation.getExperimenttime()%></center></td>
        	         <td><center><%=compensation.getTeaschool()%></center></td>
	            	 ...
	            	 <td><center><%=compensation.getPay()%></center></td>		          
	            	 </tr>
	            	 <%
	             }
	         } %>
		...
		<%-- 	[<button style="border:0;background-color: ;"  onclick="javascript:to_page(${Page.currpageno-1})">前一页</button>] --%>
			[<a href="ShowManAllCompensationServlet?page=${Page.currpageno-1}&&school=${sessionScope.school}" style="text-decoration:none">前一页</a>]
			
		<%-- 	[<button style="border:0;background-color: ;" onclick="javascript:to_page(${Page.currpageno+1})">后一页</button>] --%>
 [<a href="ShowManAllCompensationServlet?page=${Page.currpageno+1}&&school=${sessionScope.school}" style="text-decoration:none">后一页</a>]
