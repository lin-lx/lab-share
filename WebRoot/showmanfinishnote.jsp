...
.table-a table td{border-left:0;border-top:1px solid skyblue;height:40px}
 ...
 <form class="table-a">
   ...
   <center>
   <form class="table-c" action="SelectManFinishNoteServlet?school=<%=session.getAttribute("school")%>" method="post">
   ...
   <tr><td colspan="10" id="myfont1" style="background:url(images/a.jpg);height:30;">实验室信息</td></tr>
   <tr style="font-weight: bold;">
       ...
       <td align="center">卫生状况</td>
       <td align="center">破损或丢失</td>
         ...
         <% ArrayList<Note> notes=(ArrayList<Note>)application.getAttribute("notes");	
			if(notes!=null){
	          	for(int i=0;i<notes.size();i++)
	         	 {
	            	 ...
	            	 %>
	            	 <tr>
	            	 <td><center><%=note.getNumber()%></center></td>
        	         <td><center><%=note.getName()%></center></td>
        	         <td><center><%=note.getExperimentdate()%> <%=note.getExperimenttime()%></center></td>
        	         <td><center><%=note.getTeaschool()%></center></td>
        	         <td><center><%=note.getTreaty()%></center></td>
        	         <td><center><%=note.getEnvironment()%></center></td>
	            	 <td><center><%=note.getDamage()%></center></td>
	            	 <td><center><%=note.getDamagedescribe()%></center></td>
	            	 <td><center><%=note.getNotetime()%></center></td>
	            	 <td><center><a href="ShowManNoteFinishPictureServlet?number=<%=note.getNumber()%>&&school=<%=session.getAttribute("school")%>&&experimenttime=<%=note.getExperimenttime()%>&&experimentdate=<%=note.getExperimentdate()%>"style="text-decoration:none;color:black;"><img src="sysimages/photos.png" style="width:20px;height:20px"></a></center></td>
	            	 </tr>
	            	 <%
	             }
	         } %>
...
总共[<B>${Page.totalcount}</B>]记录   共[<B>${Page.totalPagecount}</B>]页
		...
		<%-- 	[<button style="border:0;background-color: ;"  onclick="javascript:to_page(${Page.currpageno-1})">前一页</button>] --%>
			[<a href="ShowManFinishNoteServlet?page=${Page.currpageno-1}&&school=${sessionScope.school}" style="text-decoration:none">前一页</a>]
			
		<%-- 	[<button style="border:0;background-color: ;" onclick="javascript:to_page(${Page.currpageno+1})">后一页</button>] --%>
 [<a href="ShowManFinishNoteServlet?page=${Page.currpageno+1}&&school=${sessionScope.school}" style="text-decoration:none">后一页</a>]
