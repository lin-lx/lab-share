...
<a href="index.jsp" style="text-decoration:none;color:white;"><img src="sysimages/relogin.png" style="width:20px;height:20px">退出登录</a>
  ...
  当前位置：赔偿信息处理
   ...
   <center>
   <form class="table-c" action="SelectTeaCompensationServlet?username=<%=session.getAttribute("username")%>" method="post">
   ...
   所属学校：<select id="myinput" name="labschool">
 				<option value=""></option>
   ...
   <tr><td colspan="10" id="myfont1" style="background:url(images/a.jpg);height:30;">赔偿信息</td></tr>
   <tr style="font-weight: bold;">
       ...
       <td align="center">记录人手机</td>
       <td align="center">记录时间</td>
       <td align="center" colspan="2">操作</td>
       </tr>
         <% ArrayList<Compensation> compensations=(ArrayList<Compensation>)application.getAttribute("compensations");	
			if(compensations!=null){
	          	for(int i=0;i<compensations.size();i++)
	         	 {
	            	 ...
	            	 %>
	            	 <tr>
	            	 <td><center><%=compensation.getNumber()%></center></td>
        	         <td><center><%=compensation.getExperimentdate()%> <%=compensation.getExperimenttime()%></center></td>
        	         <td><center><%=compensation.getLabschool()%></center></td>
        	         <td><center><%=compensation.getMoney()%></center></td>
	            	 <td><center><%=compensation.getIllustration()%></center></td>
	            	 <td><center><%=compensation.getNotename()%></center></td>
	            	 <td><center><%=compensation.getNotephone()%></center></td>
	            	 <td><center><%=compensation.getNotetime()%></center></td>
	            	 <td><center><a href="ShowTeaCompensationPictureServlet?number=<%=compensation.getNumber()%>&&labschool=<%=compensation.getLabschool()%>&&experimenttime=<%=compensation.getExperimenttime()%>&&experimentdate=<%=compensation.getExperimentdate()%>" style="text-decoration:none;color:black;"><img src="sysimages/photos.png" style="width:20px;height:20px"></a></center></td>		
	            	 <td><center><a href="ToPayCompensationServlet?number=<%=compensation.getNumber()%>&&experimenttime=<%=compensation.getExperimenttime()%>&&labschool=<%=compensation.getLabschool()%>&&experimentdate=<%=compensation.getExperimentdate()%>"style="text-decoration:none;color:black;"><img src="sysimages/pay.png" style="width:20px;height:20px">付款</a></center></td>	          
	            	 </tr>
	            	 <%
	             }
	         } %>
		...
		<%-- 	[<button style="border:0;background-color: ;"  onclick="javascript:to_page(${Page.currpageno-1})">前一页</button>] --%>
			[<a href="ShowCompensationTeaServlet?page=${Page.currpageno-1}&&username=${sessionScope.username}" style="text-decoration:none">前一页</a>]
			
		<%-- 	[<button style="border:0;background-color: ;" onclick="javascript:to_page(${Page.currpageno+1})">后一页</button>] --%>
 [<a href="ShowCompensationTeaServlet?page=${Page.currpageno+1}&&username=${sessionScope.username}" style="text-decoration:none">后一页</a>]
