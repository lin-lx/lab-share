...
public class ShowTeachersLOp {
	...
	public int getTotalCount(User user) {
		String sqlstr="select count(*) as tc from user where school='" +user.getSchool()+ "' and identity='教师' ";
...
//分页查询本校所有教师信息
			public ArrayList<User>  select(User user,int pageno,int pageSize)
			{
				ArrayList<User> retlist=new  ArrayList<User>();
				 try{
					 cn=new ConnDb().getCon();
					 String sqlstr="select  * from user where school='" +user.getSchool()+ "' and identity='教师' limit "+pageSize*(pageno-1)+","+pageSize+";";
