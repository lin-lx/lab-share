...
//查询实验室是否在使用中,如果实验室在使用中，则不可以被删除
	public ArrayList<Application>  select(Application application)
	{
		ArrayList<Application> retlist=new  ArrayList<Application>();
		 try{
			 cn=new ConnDb().getCon();
			 String sqlstr="select  * from application where number='"+application.getNumber()+"' and labschool='"+application.getLabschool()+"'and date>current_date();";
...
//删除实验室
	public boolean delete(String number,String labschool)
	{
		boolean delFlag = false;
		 try{
			 cn=new ConnDb().getCon();
	
			 String sqlstr="delete from  lab  where number='"+number+"'and labschool='"+labschool+"';";
			 //JOptionPane.showMessageDialog(null, "sql"+sqlstr);
			 ps=cn.prepareStatement(sqlstr);
