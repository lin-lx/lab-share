...
public class TeaNoteSituationResultOp {
	...
	public int getTotalCount(User user) {
		String sqlstr="select count(*) as tc from note where username='" +user.getUsername()+ "' and treaty='否' ";
	...
	public ArrayList<Note>  quire(User user,int pageno,int pageSize)
	{
		ArrayList<Note> retlist=new  ArrayList<Note>();
		 try{
			 cn=new ConnDb().getCon();
			 String sqlstr="select  * from note where username='" +user.getUsername()+ "' and treaty='否'order by notetime desc limit "+pageSize*(pageno-1)+","+pageSize+";";
