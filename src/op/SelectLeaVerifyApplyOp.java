...
public class SelectLeaVerifyApplyOp {
	...
	//按总条数模糊查询待审核信息	
		...
		public int getTotalCount(String sql1,User user) {
			String sqlstr="select count(*) as tc from application where labschool= '" +user.getSchool()+ "'and result='待审核' and cancelapply='否' "+sql1;
...
//按分页模糊查询待审核信息	
	public ArrayList<Application> select(String sql1,User user,int pageno,int pageSize) throws SQLException{
		...
		cn=new ConnDb().getCon();
		String sql = "select  * from application where labschool= '" +user.getSchool()+ "'and result='待审核' and cancelapply='否' "+sql1+ " order by date desc limit "+pageSize*(pageno-1)+","+pageSize+";";
