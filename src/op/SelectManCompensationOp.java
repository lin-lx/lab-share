...
public class SelectManCompensationOp {
	...
	//按总条数模糊查询所有赔偿消息	
		...
		public int getTotalCount(String sql1,User user) {
			String sqlstr="select count(*) as tc from compensation where labschool='" +user.getSchool()+ "' and noteusername='" +user.getUsername()+"'"+sql1;
	...
	public ArrayList<Compensation> select(String sql1,User user,int pageno,int pageSize) throws SQLException{
		...
		cn=new ConnDb().getCon();
		String sql = "select  * from compensation where labschool='" +user.getSchool()+ "' and noteusername='" +user.getUsername()+"'"+sql1+ " order by notetime desc limit "+pageSize*(pageno-1)+","+pageSize+";";
