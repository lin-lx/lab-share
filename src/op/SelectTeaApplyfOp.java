...
public class SelectTeaApplyfOp {
...
//按总数模糊查询教师所有申请的实验室	
	public int getTotalCount(String sql1,User user) {
		String sqlstr="select count(*) as tc from application where username= '" +user.getUsername()+ "' and cancelapply='否' "+sql1;
	...
	public ArrayList<Application> select(String sql1,User user,int pageno,int pageSize) throws SQLException{
		...
		cn=new ConnDb().getCon();
		String sql = "select  * from application where username= '" +user.getUsername()+ "' and cancelapply='否'"+sql1+ " order by date desc limit "+pageSize*(pageno-1)+","+pageSize+";";
