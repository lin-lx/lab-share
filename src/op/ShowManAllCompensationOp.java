...
public class ShowManAllCompensationOp {
	...
	public int getTotalCount(User user) {
		String sqlstr="select count(*) as tc from compensation where labschool='" +user.getSchool()+ "' and noteusername='" +user.getUsername()+"' ";
	...
	public ArrayList<Compensation>  quire(User user,int pageno,int pageSize)
	{
		...
		try{
			 cn=new ConnDb().getCon();
			 String sqlstr="select  * from compensation where labschool='" +user.getSchool()+ "' and noteusername='" +user.getUsername()+"'order by notetime desc limit "+pageSize*(pageno-1)+","+pageSize+";";
