...
public class SelectLeaVerifyResultOp {
	...
	//按总条数模糊查询审核结果	
		...
		public int getTotalCount(String sql1,User user) {
			String sqlstr="select count(*) as tc from application where result='审核通过' and labschool= '" +user.getSchool()+ "'and cancelapply='否'"+sql1;
	...
	public ArrayList<Application> select(String sql1,User user,int pageno,int pageSize) throws SQLException{
		...
		cn=new ConnDb().getCon();
		String sql = "select * from application where result='审核通过' and labschool= '" +user.getSchool()+ "'and cancelapply='否'"+sql1+ " order by date desc limit "+pageSize*(pageno-1)+","+pageSize+";";
