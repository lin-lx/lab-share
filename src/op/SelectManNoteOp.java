...
public class SelectManNoteOp {
	...
	//按总条数模糊查询教师所有已用实验室	
		...
		public int getTotalCount(String sql1,User user) {
			String sqlstr="select count(*) as tc from application where ((number not in(select number from note)) or (experimenttime not in(select experimenttime from note)) or (date not in(select experimentdate from note))) and labschool='"+user.getSchool()+"' and result ='审核通过' and cancelapply='否' and date<current_date() "+sql1;
	...
	public ArrayList<Application> select(String sql1,User user,int pageno,int pageSize) throws SQLException{
		...
		cn=new ConnDb().getCon();
		String sql = "select * from application where ((number not in(select number from note)) or (experimenttime not in(select experimenttime from note)) or (date not in(select experimentdate from note))) and labschool='"+user.getSchool()+"' and result ='审核通过' and cancelapply='否' and date<current_date() "+sql1+ " order by date desc limit "+pageSize*(pageno-1)+","+pageSize+";";
