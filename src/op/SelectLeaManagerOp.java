...
public class SelectLeaManagerOp {
	...
	//按总条数模糊查询本校实验室管理员	
		...
		public int getTotalCount(String sql1,User user) {
			String sqlstr="select count(*) as tc from user where school='" +user.getSchool() +"' and identity='实验室管理员' "+sql1;
	...
	public ArrayList<User> select(String sql1,User user,int pageno,int pageSize) throws SQLException{
		...
		cn=new ConnDb().getCon();
		String sql = "select  * from user where school='" +user.getSchool() +"' and identity='实验室管理员' "+sql1+ " order by registertime desc limit "+pageSize*(pageno-1)+","+pageSize+";";
