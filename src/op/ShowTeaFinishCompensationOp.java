...
public class ShowTeaFinishCompensationOp {
	...
	//按总条数查询教师所有赔偿记录
		public int getTotalCount(User user) {
			String sqlstr="select count(*) as tc from compensation where  applyusername='" +user.getUsername()+"' and pay='是' ";
...
//分页查询教师所有赔偿记录
	public ArrayList<Compensation>  quire(User user,int pageno,int pageSize)
	{
		...
		try{
			 cn=new ConnDb().getCon();
			 String sqlstr="select  * from compensation where  applyusername='" +user.getUsername()+"' and pay='是' order by paytime desc limit "+pageSize*(pageno-1)+","+pageSize+";";
