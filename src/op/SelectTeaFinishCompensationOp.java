...
public class SelectTeaFinishCompensationOp {
	...
	//按总条数模糊查询教师所有赔偿记录	
		...
		public int getTotalCount(String sql1,User user) {
			String sqlstr="select count(*) as tc from compensation where  applyusername='" +user.getUsername()+"' and pay='是'"+sql1;
	...
	public ArrayList<Compensation> select(String sql1,User user,int pageno,int pageSize) throws SQLException{
		...
		cn=new ConnDb().getCon();
		String sql = "select  * from compensation where  applyusername='" +user.getUsername()+"' and pay='是'"+sql1+ " order by paytime desc limit "+pageSize*(pageno-1)+","+pageSize+";";
	// JOptionPane.showMessageDialog(null, "sql:" +sql);
		...
		while(rs.next()){
			...
			temp.setNumber(rs.getString(1));
			 ...
			 temp.setPayment(rs.getString(19));
			 all.add(temp);		
