...
import op.ShowTeaFinishLabOp;
import bean.Application;
import bean.CodeExchange;
import bean.Page;
import bean.User;

public class ShowTeaFinishLabServlet extends HttpServlet {
	CodeExchange ce = new CodeExchange();
	/**
	 * Constructor of the object.
	 */
	public ShowTeaFinishLabServlet() {
		super();
	}
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		ArrayList<Application> applications=new ArrayList<Application>();
		 st = new ShowTeaFinishLabOp();
		user=new User();
		user.setUsername(username);
		p.setTotalcount((st.getTotalCount(user)));
		try {
	    applications =st.select(user,p.getCurrpageno(), p.getPagesize());
		} catch (Exception e) {
