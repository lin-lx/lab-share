...
import op.SelectTeaFinishCompensationOp;
import bean.CodeExchange;
import bean.Compensation;
import bean.Page;
import bean.User;

public class SelectTeaFinishCompensationServlet extends HttpServlet {
	CodeExchange ce = new CodeExchange();

	/**
	 * Constructor of the object.
	 */
	public SelectTeaFinishCompensationServlet() {
		super();
	}
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		SelectTeaFinishCompensationOp st;
		User user;
		...
		try {
			st = new SelectTeaFinishCompensationOp();
			user=new User();
			user.setUsername(username);
			p.setTotalcount((st.getTotalCount(sql,user)));
			compensations=st.select(sql,user,p.getCurrpageno(), p.getPagesize());
		} catch (Exception e) {
		...
		request.getRequestDispatcher("showteafinishcompensation.jsp").forward(request, response);	
	}
