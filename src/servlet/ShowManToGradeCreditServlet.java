...
public class ShowManToGradeCreditServlet extends HttpServlet {
	CodeExchange ce = new CodeExchange();

	/**
	 * Constructor of the object.
	 */
	public ShowManToGradeCreditServlet() {
		super();
	}
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		ArrayList<Note> notes=new ArrayList<Note>();
		 m = new ShowManToGradeCreditOp();
		user=new User();
		user.setSchool(school);
		note=new Note();
		p.setTotalcount((m.getTotalCount(user)));
		try{
		notes =m.select(note,user,p.getCurrpageno(), p.getPagesize());
