...
import op.SelectManToGradeCreditOp;
import bean.CodeExchange;
import bean.Note;
import bean.Page;
import bean.User;

public class SelectManToGradeCreditServlet extends HttpServlet {
	CodeExchange ce = new CodeExchange();

	/**
	 * Constructor of the object.
	 */
	public SelectManToGradeCreditServlet() {
		super();
	}
...
// JOptionPane.showMessageDialog(null, "sql:" +sql);
	...
	SelectManToGradeCreditOp sm;
	User user;
	String school=ce.ChineseCoding(request.getParameter("school"));
	ArrayList<Note> notes=new ArrayList<Note>();
	try {
		sm = new SelectManToGradeCreditOp();
		user=new User();
		user.setSchool(school);
		Note note;
		note=new Note();
		p.setTotalcount((sm.getTotalCount(sql,user)));
		notes=sm.select(sql,note,user,p.getCurrpageno(), p.getPagesize());
	} catch (Exception e) {
