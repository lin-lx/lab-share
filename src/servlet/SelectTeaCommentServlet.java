...
import op.SelectTeaCommentOp;
import bean.CodeExchange;
import bean.Comment;
import bean.Page;
import bean.User;

public class SelectTeaCommentServlet extends HttpServlet {
	 ...
	 */
	public SelectTeaCommentServlet() {
		super();
	}
...
public void doPost(HttpServletRequest request, HttpServletResponse response)
	...
	if (experimenttime!=null&&!"".equals(experimenttime)) {
		sql+="experimenttime like '%"+experimenttime+"%'";
	}
	
	else sql=sql.substring(0, sql.length()-4);
	
	SelectTeaCommentOp st;
	User user;
	String username=ce.ChineseCoding(request.getParameter("username"));
	
	ArrayList<Comment> comments=new ArrayList<Comment>();
	try {
		st = new SelectTeaCommentOp();
		user=new User();
		user.setUsername(username);
		p.setTotalcount((st.getTotalCount(sql,user)));
		comments=st.select(sql,user,p.getCurrpageno(), p.getPagesize());
	} catch (Exception e) {
		
		e.printStackTrace();
	}
	//JOptionPane.showMessageDialog(null, comments.size());
