...
import op.ShowManFinishNoteOp;
import bean.CodeExchange;
import bean.Note;
import bean.Page;
import bean.User;

public class ShowManFinishNoteServlet extends HttpServlet {
	CodeExchange ce = new CodeExchange();

	/**
	 * Constructor of the object.
	 */
	public ShowManFinishNoteServlet() {
		super();
	}
		...
		if(currpageno==null||"".equals(currpageno)) {
			...
			p.setCurrpageno(Integer.parseInt(currpageno.trim()));
					...
					ArrayList<Note> notes=new ArrayList<Note>();
					 sm = new ShowManFinishNoteOp();
					 user=new User();
					 user.setSchool(school);
					 p.setTotalcount((sm.getTotalCount(user)));
					 try{
					 notes = sm.quire(user,p.getCurrpageno(), p.getPagesize());
					...
					request.getRequestDispatcher("showmanfinishnote.jsp").forward(request, response);	
